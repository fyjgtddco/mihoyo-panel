...
class MihoyoPanel(Star):
    ...
    async def _ensure_font(self):
        ...
        if self.font and not isinstance(self.font, ImageFont.ImageFont):
            return
        logger.error("未找到中文字体！中文将显示为方框，请手动放置字体文件到 fonts/ 目录")
            ...
            for filename in STATIC_FILES:
                ...
                if not filepath.exists() or filepath.stat().st_size < 100:
                    ...
                    try:
                        ...
                        logger.info(f"  保存到: {filepath} ({len(data)} bytes)")
                    except Exception as e:
    ...
    async def _draw_card(self, ava: dict, game: str) -> Image.Image:
        ...
        for art in arts[:6]:
            ...
            # 等级
            lv = art.get("level") or flat.get("level", 1)
