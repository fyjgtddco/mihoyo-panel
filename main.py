...
class MihoyoPanel(Star):
    ...
    async def _draw_card(self, ava: dict, game: str) -> Image.Image:
        ...
        lv_text = f"Lv.{level}"
        # 用 draw.textlength 替代 getbbox
        lv_w = draw.textlength(lv_text, font=small_font)
        ...
        for art in arts[:6]:
            ...
            if isinstance(lv, dict):
                lv = 1
            et = flat.get("equipType", art.get("type", ""))
            rank = flat.get("rankLevel", art.get("rarity", ""))
