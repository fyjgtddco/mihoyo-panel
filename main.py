...
# 静态数据：走 ghproxy 国内镜像加速
DATA_BASE = "https://ghfast.top/https://raw.githubusercontent.com/EnkaNetwork/API-docs/master/store/"

# 字体：走 jsDelivr CDN（国内可用）
FONT_URL = "https://cdn.jsdelivr.net/gh/AimeeMao/Fonts@latest/NotoSansCJKsc-Regular.otf"
...
class MihoyoPanel(Star):
    ...
    # ------------------------------------------------------------
    # 字体（优先本地 → jsDelivr CDN 下载 → 系统字体 → 默认）
    # ------------------------------------------------------------
    async def _ensure_font(self):
        ...
        if font_path.exists() and font_path.stat().st_size > 100000:
            if self._load_font_file(font_path):
                return

        # 2. 尝试从 jsDelivr CDN 下载
        logger.info("正在从 jsDelivr CDN 下载中文字体...")
        try:
            ...
            logger.warning(f"从 CDN 下载字体失败: {e}")
        ...
        # 4. 最终兜底
        logger.error("未找到中文字体！请手动放置 .otf/.ttf 文件到 fonts/ 目录")
            ...
            for filename in STATIC_FILES:
                ...
                if not filepath.exists() or filepath.stat().st_size < 500:
                    ...
                    try:
                        ...
                        logger.info(f"  保存: {filepath} ({len(data)} bytes)")
                    except Exception as e:
                ...
                try:
                    ...
                    self.static_data[key] = content
                    if isinstance(content, list):
                        logger.info(f"  加载 {filename}: 列表, {len(content)} 条")
                    elif isinstance(content, dict):
                        logger.info(f"  加载 {filename}: 字典, keys={list(content.keys())[:5]}")
                except Exception as e:
    ...
    def _localize(self, hash_value) -> str:
        ...
        if isinstance(loc, dict):
            zh = loc.get("zh-CN") or loc.get("chs") or loc.get("zh") or {}
            result = zh.get(s, "")
            if result:
                return result
        return s
    ...
    def _get_character_name(self, avatar_id) -> str:
        ...
        if isinstance(chars, list):
            for c in chars:
                cid = str(c.get("id") or c.get("Id") or c.get("ID") or "")
                if cid == aid:
                    hash_val = c.get("NameTextMapHash", "")
                    name = self._localize(hash_val)
                    if name == str(hash_val) or not name:
                        name = c.get("en") or c.get("name") or c.get("Name") or aid
                    return name
        return aid
    ...
    def _get_character_icon(self, avatar_id) -> str:
        ...
        if isinstance(chars, list):
            for c in chars:
                cid = str(c.get("id") or c.get("Id") or c.get("ID") or "")
                if cid == aid:
                    icon = c.get("SideIconName") or c.get("IconName") or c.get("icon")
                    if icon:
                        return f"{icon}.png"
    ...
    async def _draw_card(self, ava: dict, game: str) -> Image.Image:
        ...
        logger.info(f"角色: avatar_id={avatar_id} → {name}, level={level}, equips={len(equips_raw)}件")
        ...
        if wp:
            ...
            # 武器等级：优先 eq.level，再尝试从各子对象取
            wlv = wp.get("level") or flat.get("level", 1)
            if wlv is None or isinstance(wlv, dict):
                # 原神武器等级在 weapon.level
                if wp.get("weapon"):
                    wlv = wp["weapon"].get("level", 1)
            if isinstance(wlv, dict):
                wlv = wlv.get("val", wlv.get("level", 1))
            if wlv is None:
                wlv = 1
            logger.info(f"  武器: {wname} Lv.{wlv}")
        ...
        for art in arts[:6]:
            ...
            # 等级：优先 eq.level，其次 flat.level，再尝试 reliquary.level / relic.level
            lv = art.get("level")
            if lv is None or isinstance(lv, dict):
                lv = flat.get("level")
            if lv is None or isinstance(lv, dict):
                # 原神：eq.reliquary.level，星铁：eq.relic?.level，绝区零：eq.level 应该已取到
                for sub_key in ("reliquary", "relic", "equipment"):
                    sub = art.get(sub_key, {})
                    if isinstance(sub, dict) and sub.get("level") is not None:
                        lv = sub["level"]
                        break
            ...
            logger.info(f"  圣遗物: {set_name} {et} +{lv}")
