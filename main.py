...
# ------------------------------------------------------------
# Enka 枚举值 → 中文映射表
# （这些是 Enka API 直接返回的枚举键名，不是哈希值，不能用 loc.json 查）
# ------------------------------------------------------------
EQUIP_TYPE_MAP = {
    "EQUIP_BRACER": "生之花",
    "EQUIP_NECKLACE": "死之羽",
...
class MihoyoPanel(Star):
    ...
    async def on_start(self):
        ...
        # 使用默认 headers，aiohttp 会自动处理 gzip 解压
    ...
    async def _fetch(self, url: str) -> bytes:
        """获取 URL 内容，自动处理 gzip 解压"""
        proxy = self.config.get("proxy", "").strip()
        kwargs = {"proxy": proxy} if proxy else {}
        async with self.session.get(url, **kwargs) as resp:
            resp.raise_for_status()
            data = await resp.read()
            # 如果 aiohttp 没有自动解压，手动解压
    ...
    async def _query(self, event: AstrMessageEvent, game: str):
        ...
        try:
            ...
            async with self.session.get(url, proxy=proxy) as resp:
                ...
                # 使用 read() 获取原始字节，再用 _fetch 里的方式手动确保解压
