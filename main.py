...
class MihoyoPanel(Star):
    ...
    async def handle_command(self, event: AstrMessageEvent, *args, **kwargs):
        ...
        # 匹配解绑指令 /解绑原神 等
        for keyword, game in UNBIND_KEYWORDS.items():
            if f"/{keyword}" in text:
                event.stop_event()
                await self._do_unbind(event, game)
                return

        # 匹配绑定指令 /原神 123456 等
        for keyword, game in GAME_KEYWORDS.items():
            pattern = rf"/{keyword}\s+(\d+)"
            match = re.search(pattern, text)
            if match:
                uid = match.group(1)
                event.stop_event()
                await self._do_bind(event, game, uid)
                return

        # 匹配查询指令 /查询原神面板 等
        for keyword, game in QUERY_KEYWORDS.items():
            if f"/{keyword}" in text:
                event.stop_event()
                await self._do_query(event, game)
                return
    ...
    # 绑定
    # ------------------------------------------------------------
    async def _do_bind(self, event: AstrMessageEvent, game: str, uid: str):
        if not uid.isdigit():
            await self._send_text(event, "❌ UID 必须为纯数字")
            return
        ...
        await self._send_text(event, f"✅ {GAME_NAMES[game]} UID 绑定成功：{uid}")

    # ------------------------------------------------------------
    # 解绑
    # ------------------------------------------------------------
    async def _do_unbind(self, event: AstrMessageEvent, game: str):
        group_id = str(event.message_obj.group_id) if event.message_obj.group_id else "private"
        user_id = str(event.message_obj.sender.user_id)
        binds = self._get_binds()
        user_binds = binds.get(group_id, {}).get(user_id, {})
        if game in user_binds:
            del user_binds[game]
            self._save_binds(binds)
            await self._send_text(event, f"✅ 已解绑{GAME_NAMES[game]}账号")
        else:
            await self._send_text(event, f"❌ 你还没有绑定{GAME_NAMES[game]}账号")

    # ------------------------------------------------------------
    # 查询
    # ------------------------------------------------------------
    async def _do_query(self, event: AstrMessageEvent, game: str):
        if self.session is None:
            await self._send_text(event, "❌ 插件正在初始化，请稍后再试")
            return
        ...
        if not uid:
            await self._send_text(event, f"❌ 请先使用 /{GAME_NAMES[game]} <UID> 绑定你的{GAME_NAMES[game]}账号")
            return
        ...
        try:
            async with self.session.get(url, proxy=self.config.get("proxy", "").strip() or None) as resp:
                if resp.status != 200:
                    txt = await resp.text()
                    await self._send_text(event, f"❌ 查询失败（{resp.status}）：可能展柜未公开或UID不存在。\n{txt[:100]}")
                    return
                data = await resp.json()
        except Exception as e:
            logger.error(f"网络请求失败: {e}")
            await self._send_text(event, f"❌ 网络请求失败: {e}")
            return
        ...
        if not avatar_list:
            ...
            await self._send_text(event, f"玩家 {nickname} 的角色展柜为空或未公开详情。")
            return

        nickname = player_info.get("nickname", "未知")
        await self._send_text(event, f"正在生成 {nickname} 的{GAME_NAMES[game]}角色面板，请稍候...")
        ...
        for idx, avatar in enumerate(avatar_list):
            try:
                ...
                await self._send_text(event, f"⚠️ 角色 {idx+1} 生成失败：{e}")
